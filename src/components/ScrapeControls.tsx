...
export function ScrapeControls({ liveSource }: { liveSource: boolean }) {
  ...
  async function scrape(body: Record<string, unknown>, label: string) {
    ...
    try {
      ...
      const found = data.runs.reduce((n: number, r: { found: number }) => n + r.found, 0);
      const src = data.runs[0]?.source ?? "";
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span
          ...
          title={liveSource ? "GOOGLE_PLACES_API_KEY detected" : "No API key — using demo seed data"}
        >
          {liveSource ? "Google Places: live" : "Demo seed data"}
        </span>
      </div>
