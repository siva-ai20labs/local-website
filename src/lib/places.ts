import type { ScraperAdapter, ScrapedBusiness, ScrapedReview } from "./types";

/**
 * Google Places API (v1) adapter.
 *
 * Uses the modern `places:searchText` endpoint, which returns business
 * details, photos, and a handful of reviews in a single call. Requires
 * GOOGLE_PLACES_API_KEY with the "Places API (New)" enabled.
 *
...
export class GooglePlacesAdapter implements ScraperAdapter {
  ...
  async search(category: string, location: string, limit: number): Promise<ScrapedBusiness[]> {
    ...
    const res = await fetch(SEARCH_URL, {
      ...
      // The scraper is a background job — don't let Next cache it.
      cache: "no-store",
    });
...
export function getGooglePlacesAdapter(): GooglePlacesAdapter | null {
