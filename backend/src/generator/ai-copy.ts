...
import type { Branding, SiteCopy } from "@local-website/types";
 ...
 *
 * Falls back to deterministic copy when no API key is configured or the API
 * errors, so site generation always succeeds.
 *
 * On Cloudflare Workers there is no `process.env`, so the API key and model are
 * passed in explicitly from the Worker `env` binding.
 */
...
export async function generateCopy(input: CopyInput, opts: CopyOptions = {}): Promise<CopyResult> {
